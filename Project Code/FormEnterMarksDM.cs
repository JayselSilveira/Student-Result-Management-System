...
{
    public partial class FormEnterMarksDM : Form
    {
        Form parentForm;

        public FormEnterMarksDM(Form form)
        {
            InitializeComponent();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(textBox1.Text) && !string.IsNullOrEmpty(textBox2.Text))
            {
                try
                {
                    OleDbConnection con = new OleDbConnection("Provider = Microsoft.ACE.OLEDB.12.0; Data Source = 'D:\\StudentResultManagementSystem.xlsx'; Extended Properties = Excel 8.0;");
                    con.Open();
                    String sql = "UPDATE [Semester4$] SET Discrete_Mathematics = @DM where StudentId = @ID";
                    OleDbCommand cmd = new OleDbCommand(sql, con);
                    cmd.Parameters.Add(new OleDbParameter("@DM", textBox2.Text));
